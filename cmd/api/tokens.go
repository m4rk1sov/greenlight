...
func (app *application) createAuthenticationTokenHandler2(w http.ResponseWriter, r *http.Request) {
	// Parse the email and password from the request body.
	var input struct {
		Email    string `json:"email"`
		Password string `json:"password"`
	...
	// Validate the email and password provided by the client.
	v := validator.New()
	data.ValidateEmail2(v, input.Email)
	data.ValidatePasswordPlaintext2(v, input.Password)
	if !v.Valid() {
		app.failedValidationResponse(w, r, v.Errors)
		return
	}
